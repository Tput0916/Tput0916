...
# --- Functions extract_script_data and perform_comparison remain unchanged ---
# (The code for these two functions is identical to what you provided, so I'm omitting it here for brevity.
# Just make sure they are included in your file.)
...
def main():
    ...
    # Clean up the path, which is especially important for drag-and-drop on macOS
    target_folder_input = input("Please drag the folder containing your data here and press Enter: ")
    target_folder = target_folder_input.strip().strip('\'"').strip()

    if not os.path.isdir(target_folder):
        print(f"\nError: The provided path '{target_folder}' is not a valid directory.")
        input("Press Enter to exit.") # Keeps window open so user can see the error
        return

    # *** MODIFICATION 1: Output directory is now inside the user's data folder ***
    ...
    # ... (summary table, drop rate, report generation code is all here) ...
    ...
    # *** MODIFICATION 2: Add a pause at the end ***
